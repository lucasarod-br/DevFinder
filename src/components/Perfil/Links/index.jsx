...
const Links = ({location, blog, twitter, company}) => {
  ...
  return (
    ...
    <StyledLinks>
        ...
        <Link inavaible={!location}>
            <FiLink />
            <TextBody>  {blog ? <a href={'https://www.' + blog} target="_blank">{blog}</a> : errorMsg } </TextBody>
        </Link>
        <Link inavaible={!location}>
            <FaTwitter />
            <TextBody> {twitter ? <a href={`https://www.twitter.com/${twitter}`}  target="_blank"> @{twitter}</a> : errorMsg} </TextBody>
        </Link>
        <Link inavaible={!location}>
            <HiBuildingOffice2 />
